...
export default function SearchScreen() {
  ...
  const isEmpty = !loading && searchText.trim().length > 0 && stories.length === 0 && profiles.length === 0;
                ...
                <View style={{ gap: 8 }}>
                  {stories.map(story => (
                    ...
                    >
                      <View style={s.card}>
                        {story.cover_url && (
                          <Image source={{ uri: story.cover_url }} style={s.cardImage} />
                        )}
...
const s = StyleSheet.create({
  ...
  cardAuthor: { color: C.textSecondary, fontSize: 12 },
  profileCard: {
    backgroundColor: C.card,
    borderRadius: 12,
    borderWidth: 1,
